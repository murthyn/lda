...
def test(k, iterations):
	...
	allmydocs = getalldocs("alldocs.txt")
	vocab = getVocab("vocab.csv")

	# vocab = getVocab("dictionary2.csv")
	testset = SVILDA(vocab = vocab, K = k, D = 16294, alpha = 0.2, eta = 0.2, tau = 1024, kappa = 0.7, docs = allmydocs, iterations= iterations)
	testset.runSVI()

	finallambda = testset._lambda
	
	heldoutdocs = getalldocs("alldocs2.txt")
	...
	with open("temp/%i_%i_%f_results.csv" %(k, iterations, perplexity), "w+") as f:
		...
		for i in tqdm(range(k)):
			...
			for j, word in enumerate(bestwords):
				# writer.writerow([word, vocab[word]])
				try:
					writer.writerow([word, [_ for _ in vocab.keys()][[_ for _ in vocab.values()].index(word)]])
					# writer.writerow([word, vocab.keys()[vocab[word]]])
				except:
					print('failed')
					print(word)
					raise Exception
	...
	for kk in range(0, len(finallambda)):
		...
		for i in range(0, 10):
			try:
				print('%20s  \t---\t  %.4f' % ([_ for _ in vocab.keys()][[_ for _ in vocab.values()].index(temp[i][1])], temp[i][0]))
			except:
				print('failed2')
				print(word)
				raise Exception
...
def main():
	...
	parser.add_argument('-v','--vocab', help='Vocab file name, .csv', default = "vocab.csv", required=False)
